#include "wsg_driver/gripper_standard_action_server.h"
  ...
  if (this->action_state.state == ActionStateCode::DONE)
  {
    ...
    else
    {
      ...
      if (this->action_state.ignore_axis_blocked && this->action_state.return_code == E_AXIS_BLOCKED)
      {
        ...
        printf("[GripperStandardActionServer::doWork] received E_AXIS_BLOCKED, but stop_on_block was false -> will "
               "acknowledge gripper error\n");
        try
        {
...
{
  this->action_server = new StandardActionServer(
      this->node_handle, this->base_name, boost::bind(&GripperStandardActionServer::goalCallback, this, _1),
...
void GripperStandardActionServer::handleCommand(control_msgs::GripperCommand command, GoalHandleStandard& goal_handle)
{
  ...
  try
  {
    // this->gripper_com.move(command.position, this->speed, this->stop_on_block,  [&](std::shared_ptr<CommandError>
    // error, std::shared_ptr<Message> message) {
    // this->commandCallback(error, message);
    // });

    this->gripper_com.custom_position(command.position, this->speed,
                                      [&](std::shared_ptr<CommandError> error, std::shared_ptr<Message> message) {
                                        this->commandCallback(error, message);
                                      });
  }
...
control_msgs::GripperCommandResult GripperStandardActionServer::fillStatus()
{
  ...
  if (this->command_position < gripperState.width + position_tolerance &&
      this->command_position > gripperState.width - position_tolerance)
